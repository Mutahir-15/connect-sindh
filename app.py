...
current_url = st.experimental_get_query_params().get("path", ["/"])[0]
...
if current_url == "/oauth2callback":
    if not is_authenticated():
        handle_oauth_callback()  # Process the OAuth callback
    st.experimental_set_query_params(path="/")  # Redirect to home after processing
    st.rerun()
